...
// Expected endpoints (protect these server-side):
//   GET /api/dune/results?queryId=XXXX  -> { rows: any[], executed_at?: string }
//   GET /api/price/auki                -> { price: number, source: string, asOf?: string }
//   GET /api/mexc/ticker?symbol=...    -> passthrough JSON (optional)
// Dune API key must NOT be in the browser. Use a server-side proxy endpoint instead.
...
async function fetchDune(queryId) {
  const url = `${API_BASE}/api/dune/results?queryId=${encodeURIComponent(queryId)}`;
  const res = await fetch(url, { headers: { "Accept": "application/json" } });
  if (!res.ok) {
    const t = await res.text().catch(() => "");
    throw new Error(`Dune proxy error ${res.status}: ${t.slice(0,200)}`);
  }
  const data = await res.json();
  // allow either {rows:[...]} or raw rows array
  const rows = Array.isArray(data) ? data : (data.rows || []);
  return { rows, executed_at: data.executed_at || null };
}


// ─── CoinGecko price ──────────────────────────────────────────────────────────
async function fetchAukiPrice() {
  const url = `${API_BASE}/api/price/auki`;
  const res = await fetch(url, { headers: { "Accept": "application/json" } });
  if (!res.ok) {
    const t = await res.text().catch(() => "");
    throw new Error(`Price endpoint error ${res.status}: ${t.slice(0,200)}`);
  }
  const data = await res.json();
  const p = Number(data?.price);
  if (!Number.isFinite(p) || !(p > 0)) throw new Error("Invalid price payload");
  return { price: p, source: data?.source || "unknown", asOf: data?.asOf || null };
}


// ─── MEXC ticker ─────────────────────────────────────────────────────────────
async function fetchMexcTicker() {
  // Prefer a server-side proxy to avoid CORS/rate-limit issues.
  // If you don't implement /api/mexc/ticker, you can remove this panel or proxy directly.
  const url = `${API_BASE}/api/mexc/ticker?symbol=AUKIUSDT`;
  try {
    const res = await fetch(url, { headers: { "Accept": "application/json" } });
...
function StatCard({ label, value, sub, accent, loading }) {
  return (
    <div style={{ background:"#111", border:"1px solid #1E1E1E", borderRadius:8, padding:"22px 26px", flex:1, minWidth:0 }}>
      <div style={{ color:"#444", fontSize:13, letterSpacing:"0.12em", textTransform:"uppercase", ...M, marginBottom:8 }}>{label}</div>
      <div style={{ color:accent||"#F0ECE3", fontSize:24, fontWeight:700, ...M, letterSpacing:"-0.02em" }}>
        {loading ? <span style={{color:"#333"}}>···</span> : value}
      </div>
      {sub && <div style={{ color:"#555", fontSize:13, marginTop:4, ...M }}>{sub}</div>}
    </div>
  );
}
...
function Loader({ msg }) {
  return (
    <div style={{ textAlign:"center", padding:"60px 0", color:"#333", ...M, fontSize:14, letterSpacing:"0.1em" }}>
    ...
    <div style={{ background:"#0F0F0F", border:"1px solid #1E1E1E", borderRadius:8, overflow:"hidden", marginBottom:16 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 20px", borderBottom:"1px solid #1A1A1A" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <span style={{ ...M, fontSize:12, color:"#333" }}>CENTRALISED EXCHANGE</span>
        </div>
      ...
      {loading ? (
        <div style={{ padding:"30px 20px", textAlign:"center", color:"#333", ...M, fontSize:14 }}>FETCHING MEXC DATA…</div>
      ) : error ? (
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 140px 80px", gap:12, padding:"0 20px 6px 36px" }}>
        {["VAULT","BALANCE","% SUPPLY"].map((h,i) => (
          <div key={h} style={{ fontSize:11, color:"#333", letterSpacing:"0.1em", ...M, textAlign:i===0?"left":"right" }}>{h}</div>
        ))}
      </div>
      {VAULT_GROUPS.map(g => {
          ...
          <div key={g.id} style={{ background:"#0F0F0F", border:"1px solid #1E1E1E", borderRadius:8, overflow:"hidden" }}>
            ...
            >
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                ...
                <span style={{ color:"#2E2E2E", ...M, fontSize:13 }}>{activeCount}/{g.vaults.length} active</span>
              </div>
            ...
            {isExp && gvaults.map((v,i) => {
              ...
              return (
                <div key={v.name}
                  ...
                  onMouseEnter={e=>{ if(!empty) e.currentTarget.style.background="#131313"; }}
                  onMouseLeave={e=>{ e.currentTarget.style.background="transparent"; }}
                >
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
...
function MovementLogTab({ movements, mode, price }) {
  ...
  const filtered = movements.filter(m => {
    if (now - new Date(m.block_time) > msOf[period]) return false;
      ...
      <div style={{ background:"#0F0F0F", border:"1px solid #1E1E1E", borderRadius:8, overflow:"hidden" }}>
        <div style={{ display:"grid", gridTemplateColumns:"150px 110px 1fr 1fr 130px", gap:12, padding:"14px 20px", borderBottom:"1px solid #1A1A1A" }}>
          {["DATE","TYPE","FROM","TO","AMOUNT"].map(h=>(
            <div key={h} style={{ fontSize:11, color:"#333", letterSpacing:"0.1em", ...M }}>{h}</div>
          ))}
        </div>
        {filtered.length===0
          ? <div style={{ padding:"40px 20px", textAlign:"center", color:"#333", ...M, fontSize:14 }}>No movements in this period</div>
          : filtered.map((m,i) => {
            const type  = typeOf(m);
            const color = TC[type];
            return (
              <div key={i}
                ...
                onMouseLeave={e=>e.currentTarget.style.background="transparent"}
              >
                <div style={{ color:"#555" }}>{fmtDate(m.block_time)}</div>
                <div><span style={{ color, fontSize:12, fontWeight:700, letterSpacing:"0.07em", padding:"2px 6px", background:`${color}18`, borderRadius:3 }}>{TL[type]}</span></div>
...
function MarketActivityTab({ daily, recentTrades }) {
  ...
  const filteredTrades = recentTrades.filter(t => now - new Date(t.block_time) <= msOf[period]);

  return (
    <div>
      {/* MEXC Section — always shown at top */}
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:12, color:"#333", letterSpacing:"0.12em", ...M, marginBottom:10 }}>CENTRALISED EXCHANGE</div>
        <MexcSection />
      </div>

      {/* DEX section */}
      <div style={{ fontSize:12, color:"#333", letterSpacing:"0.12em", ...M, marginBottom:10 }}>DECENTRALISED EXCHANGE (ON-CHAIN · BASE)</div>
      ...
      {chart.length > 1 && (
        <div style={{ background:"#0F0F0F", border:"1px solid #1E1E1E", borderRadius:8, padding:"16px 20px", marginBottom:16 }}>
          <div style={{ fontSize:12, color:"#333", letterSpacing:"0.1em", ...M, marginBottom:12 }}>PRICE (AUKI / USD) — DAILY</div>
          <div style={{ position:"relative" }}>
            ...
            <div style={{ position:"absolute", right:0, top:0, fontSize:12, color:"#555", ...M }}>{fmtPrice(maxP)}</div>
            <div style={{ position:"absolute", right:0, bottom:0, fontSize:12, color:"#555", ...M }}>{fmtPrice(minP)}</div>
          </div>
      ...
      {(totalBuyVol+totalSellVol)>0 && (
        <div style={{ background:"#0F0F0F", border:"1px solid #1E1E1E", borderRadius:8, padding:"14px 20px", marginBottom:16 }}>
          <div style={{ fontSize:12, color:"#333", letterSpacing:"0.1em", ...M, marginBottom:10 }}>BUY / SELL PRESSURE</div>
      ...
      <div style={{ background:"#0F0F0F", border:"1px solid #1E1E1E", borderRadius:8, overflow:"hidden", marginBottom:16 }}>
        ...
        {[...filteredDaily].reverse().slice(0,1000).map((d,i,arr)=>(
          <div key={i}
            style={{ display:"grid", gridTemplateColumns:"120px 110px 110px 120px 120px 80px", gap:12, padding:"12px 20px", borderBottom:i<arr.length-1?"1px solid #141414":"none", ...M, fontSize:14 }}
            onMouseEnter={e=>e.currentTarget.style.background="#131313"}
            onMouseLeave={e=>e.currentTarget.style.background="transparent"}
          >
            <div style={{ color:"#555" }}>{fmtDateShort(d.day)}</div>
      ...
      {filteredTrades.length > 0 && (
        <div style={{ background:"#0F0F0F", border:"1px solid #1E1E1E", borderRadius:8, overflow:"hidden" }}>
          <div style={{ padding:"12px 20px", borderBottom:"1px solid #1A1A1A", fontSize:12, color:"#333", letterSpacing:"0.1em", ...M }}>
          ...
          {filteredTrades.slice(0,100).map((t,i)=>{
            ...
            return (
              <div key={i}
                ...
                onMouseLeave={e=>e.currentTarget.style.background="transparent"}
              >
                <div style={{ color:"#555" }}>{fmtDate(t.block_time)}</div>
                <div><span style={{ color, fontSize:12, fontWeight:700, padding:"2px 6px", background:`${color}18`, borderRadius:3 }}>{buy?"BUY":"SELL"}</span></div>
                <div style={{ color, fontWeight:700 }}>{fmtTokens(t.auki_amount)}</div>
                <div style={{ color:"#666" }}>{fmtUsd(t.amount_usd)}</div>
                <div style={{ color:"#555" }}>{fmtPrice(t.price_per_auki)}</div>
              </div>
            );
          })}
        </div>
...
// Access control should be enforced by your hosting layer (shared password / SSO), not in client JS.

export default function AukiTreasury() {  const [mode,       setMode]       = useState("tokens");
  ...
  const [loading,    setLoading]    = useState({ balances:true, movements:true, trades:true, daily:true });
  ...
  const doFetchPrice = useCallback(async () => {
    ...
    try {
      ...
      setPriceTs(result.asOf ? new Date(result.asOf) : new Date());
      setPriceState(result.source || "live");
    } catch {
      setPrice(null);
      setPriceTs(null);
      setPriceState("error");
  ...
  useEffect(() => {
    ...
    fetchDune(QUERY_BALANCES).then(({ rows }) => {
      const map = {};
      rows.forEach(r => { const n = ADDR_TO_VAULT[r.address?.toLowerCase()]; if(n) map[n] = safeNum(r.balance); });
      setBalances(map);
      setLoading(p=>({...p,balances:false}));
    }).catch(() => setLoading(p=>({...p,balances:false})));

    fetchDune(QUERY_MOVEMENTS).then(({ rows }) => {
      setMovements(rows.map(r=>({ ...r, amount: safeNum(r.amount) })));
      setLoading(p=>({...p,movements:false}));
    }).catch(() => setLoading(p=>({...p,movements:false})));

    fetchDune(QUERY_TRADES).then(({ rows }) => {
      setTrades(rows.map(r=>({ ...r, amount_usd: safeNum(r.amount_usd), auki_amount: safeNum(r.auki_amount), price_per_auki: safeNum(r.price_per_auki) })));
      setLoading(p=>({...p,trades:false}));
    }).catch(() => setLoading(p=>({...p,trades:false})));

    fetchDune(QUERY_DAILY).then(({ rows }) => {
      setDaily(rows.map(r=>({ ...r, volume_usd: safeNum(r.volume_usd), buy_volume: safeNum(r.buy_volume), sell_volume: safeNum(r.sell_volume), low: safeNum(r.low), high: safeNum(r.high), trade_count: safeNum(r.trade_count) })));
      setLoading(p=>({...p,daily:false}));
    }).catch(() => setLoading(p=>({...p,daily:false})));
  }, []);

  const totalHeld   = Object.values(balances).reduce((s,v)=>s+safeNum(v),0);
  const activeCount = Object.values(balances).filter(v=>safeNum(v)>0).length;
  const dp          = price||0;
    ...
    <div style={{ minHeight:"100vh", background:"#080808", color:"#F0ECE3", ...M }}>
      ...
      <div style={{ borderBottom:"1px solid #161616", padding:"13px 32px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, background:"#080808", zIndex:10 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            ...
            <span style={{ fontSize:13, color:"#3A3A3A", letterSpacing:"0.08em" }}>DATA · DUNE (Base)</span>
          </div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          {priceState==="loading"
            ...
            : <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                ...
                {priceState==="error" && <span style={{ color:"#FF6B6B", fontSize:12, ...M }}>ERR</span>}
      ...
      <div style={{ padding:"26px 32px", maxWidth:1400, margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", gap:10, marginBottom:22 }}>
          ...
          <StatCard label="24H DEX Volume"   value={fmtUsd(trades24h.reduce((s,t)=>s+(safeNum(t.amount_usd)),0))}          sub={`${trades24h.length} on-chain trades`}                 loading={loading.trades}/>
          <StatCard label="Live Price"       value={priceState==="loading"?"…":fmtPrice(price)}                       sub="via /api/price/auki"                           accent="#C8A96E" loading={priceState==="loading"}/>
        </div>

        {/* Allocation bar */}
        <div style={{ background:"#111", border:"1px solid #1E1E1E", borderRadius:8, padding:"14px 20px", marginBottom:18 }}>
          <div style={{ fontSize:12, color:"#333", letterSpacing:"0.1em", marginBottom:10 }}>AUKI SUPPLY ALLOCATION</div>
          <div style={{ display:"flex", height:5, borderRadius:3, overflow:"hidden", background:"#1A1A1A" }}>
            {VAULT_GROUPS.map(g => {
              ...
              if(!pct) return null;
              return <div key={g.id} style={{ width:`${pct}%`, background:g.color, opacity:0.7 }} title={`${g.label}: ${pct.toFixed(1)}%`}/>;
            })}
          </div>
            ...
            {VAULT_GROUPS.map(g => {
              ...
              return (
                <div key={g.id} style={{ display:"flex", alignItems:"center", gap:5 }}>
                  <div style={{ width:6, height:6, borderRadius:"50%", background:g.color, opacity:pct===0?0.25:1 }}/>
                  <span style={{ fontSize:12, color:pct===0?"#2E2E2E":"#555" }}>{g.label} {pct>0?`${pct.toFixed(1)}%`:"0%"}</span>
                </div>
              );
            })}
          </div>
